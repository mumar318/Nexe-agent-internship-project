# streamlit_app.py — Streamlit Cloud entry point for Multi-Tool Agent
...
def web_search(query):
    ...
    if not key:
        return "❌ SERPAPI_KEY not set in secrets."
    try:
...
def save_note(title, content):
    try:
        ...
        conn.execute("INSERT INTO notes (title, content, timestamp) VALUES (?, ?, ?)",
...
def send_email(to, subject, body):
    ...
    pwd  = _get("EMAIL_PASSWORD")
    if not addr or not pwd:
        return "❌ EMAIL_ADDRESS or EMAIL_PASSWORD not set in secrets."
    try:
...
""")
    user_input = st.text_input("Enter your query:", placeholder="e.g. Search for latest AI news")
        ...
        if not user_input.strip():
            ...
            with st.spinner("Working on it..."):
                try:
                    ...
                    SYSTEM_PROMPT = """You are a strict tool-calling AI with three tools.
...
FORMAT: {"name": "tool_name", "arguments": {"param": "value"}}"""
                    ...
                    if tool not in TOOL_MAP:
                        ...
                        with st.expander("🔍 Raw response"):
                            st.json({"tool_used": tool, "input": args, "result": result})
...
with tab2:
    ...
    try:
        ...
        if not rows:
            ...
            for row in rows:
                with st.expander(f"📌 {row[1]}  —  {row[3][:19]}"):
                    st.write(row[2])
    except Exception as e:
